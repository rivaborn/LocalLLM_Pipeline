...
The legacy debug pipeline calls `interfaces_local.ps1` as step 2 of its 6-step sequence:
...
| `LLM_MODEL_HIGH_CTX` | fallback to `LLM_MODEL` | Preferred model name. High-context variant preferred because the synthesis pass reads ~10k tokens of per-file contracts. |
