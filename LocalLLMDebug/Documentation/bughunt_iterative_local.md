...
The legacy `Arch_Debug_Pipeline.ps1` does not call `bughunt_iterative_local.ps1` directly -- it calls the simpler `bughunt_local.ps1` instead. The iterative script is designed for standalone use when you want the LLM to attempt automatic fixes rather than just report bugs.

### Via ArchPipeline.py

The unified `ArchPipeline.py` debug mode is not yet fully wired. When completed, it is expected to call this script as a subprocess.
...
| `LLM_MODEL_HIGH_CTX` | fallback to `LLM_MODEL` | Model name. Prefers a high-context variant since fix calls need ~12k context. |
| `LLM_MODEL` | `qwen2.5-coder:14b` | Fallback model name if `LLM_MODEL_HIGH_CTX` is not set. |
