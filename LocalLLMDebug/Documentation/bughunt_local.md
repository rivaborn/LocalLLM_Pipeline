...
The legacy debug pipeline calls `bughunt_local.ps1` as step 4 of its 6-step sequence:
...
It is invoked as a subprocess with the `-TargetDir` and optional `-EnvFile` parameters forwarded from the pipeline's own arguments.
